 ...
 *      INCLUDES
 *********************/
#include "../../../Third_Party/lvgl/src/lvgl.h" /*To see all the widgets*/

#if 1
/*0: Light mode; 1: Dark mode*/
#define LV_THEME_TARAZ_DARK 0
/*1: Enable grow on press*/
#define LV_THEME_TARAZ_GROW 1
/*Default transition time in [ms]*/
#define LV_THEME_TARAZ_TRANSITION_TIME 80

#include "lv_theme_taraz.h"
#include "../../../Third_Party/lvgl/src/misc/lv_gc.h"
...
/*SCREEN*/
#define LIGHT_COLOR_SCR        lv_color_white()
#define LIGHT_COLOR_CARD       lv_color_white()
#define LIGHT_COLOR_TEXT       lv_color_black()
#define LIGHT_COLOR_GREY       lv_palette_lighten(LV_PALETTE_GREY, 2)
#define LIGHT_COLOR_OBJ		   lv_color_make(0, 200, 200)
#define LIGHT_COLOR_OBJ_BORDER lv_color_make(0, 100, 100)
#define DARK_COLOR_SCR         lv_color_make(10, 20, 60)
#define DARK_COLOR_CARD        lv_color_make(10, 20, 60)
#define DARK_COLOR_TEXT        lv_color_white()
#define DARK_COLOR_GREY        lv_color_hex(0x2f3237)
#define DARK_COLOR_OBJ		   lv_color_make(0, 100, 100)
#define DARK_COLOR_OBJ_BORDER  lv_color_make(0, 200, 200)

#define TRANSITION_TIME         LV_THEME_TARAZ_TRANSITION_TIME
...
static lv_color_t color_obj_bg;
static lv_color_t color_obj_border;
static bool inited = false;
/**********************
 *   GLOBAL VARIABLES
 **********************/
theme_colors_t themeColors;
...
static void style_init(void)
{
    ...
    themeColors.text = color_text = theme.flags & MODE_DARK ? DARK_COLOR_TEXT : LIGHT_COLOR_TEXT;
    themeColors.bg = color_card = theme.flags & MODE_DARK ? DARK_COLOR_CARD : LIGHT_COLOR_CARD;
    color_grey = theme.flags & MODE_DARK ? DARK_COLOR_GREY : LIGHT_COLOR_GREY;
    theme.color_primary = themeColors.btn = color_obj_bg = theme.flags & MODE_DARK ? DARK_COLOR_OBJ : LIGHT_COLOR_OBJ;
    themeColors.border = color_obj_border = theme.flags & MODE_DARK ? DARK_COLOR_OBJ_BORDER : LIGHT_COLOR_OBJ_BORDER;
    theme.color_secondary = themeColors.alt_btn = (theme.flags & MODE_DARK) == 0 ? DARK_COLOR_OBJ : LIGHT_COLOR_OBJ;
    themeColors.alt_border = (theme.flags & MODE_DARK) == 0 ? DARK_COLOR_OBJ_BORDER : LIGHT_COLOR_OBJ_BORDER;
    themeColors.alt_text = (theme.flags & MODE_DARK) == 0 ? DARK_COLOR_TEXT : LIGHT_COLOR_TEXT;
    themeColors.current = lv_color_make(210, 70, 0);
    themeColors.voltage = lv_color_make(0, 90, 160);
    themeColors.gray = lv_color_make(127, 127, 127);
    ...
    lv_style_set_bg_color(&styles->btn, color_obj_bg);
    ...
    lv_style_set_border_color(&styles->btn, color_obj_border);
    lv_style_set_border_width(&styles->btn, 2);
    ...
    lv_style_set_text_color(&styles->bg_color_primary, themeColors.text);
    ...
    lv_style_set_text_color(&styles->bg_color_secondary, themeColors.alt_text);
    ...
    lv_style_set_border_color(&styles->ta_cursor, themeColors.alt_text);
    ...
    lv_style_set_text_color(&styles->ta_placeholder, themeColors.alt_btn);
#endif
    ...
    lv_style_set_bg_color(&styles->msgbox_backdrop_bg, theme.color_primary);
...
#if LV_USE_WIN
    style_init_reset(&styles->win);
    lv_style_set_border_color(&styles->win, themeColors.alt_btn);
    lv_style_set_border_width(&styles->win, 2);
    lv_style_set_radius(&styles->win, 10);
#endif
}
/**********************
 *   GLOBAL FUNCTIONS
...
static void theme_apply(lv_theme_t * th, lv_obj_t * obj)
{
    ...
    if(lv_obj_check_type(obj, &lv_obj_class)) {
        ...
        lv_obj_add_style(obj, &styles->bg_color_secondary, 0);
        lv_obj_add_style(obj, &styles->pad_small, 0);
        lv_obj_add_style(obj, &styles->disabled, LV_STATE_DISABLED);
        lv_obj_add_style(obj, &styles->outline_primary, LV_STATE_FOCUS_KEY);
        lv_obj_add_style(obj, &styles->outline_secondary, LV_STATE_EDITED);
        ...
        lv_obj_add_style(obj, &styles->btn, LV_PART_ITEMS);
        lv_obj_add_style(obj, &styles->disabled, LV_PART_ITEMS | LV_STATE_DISABLED);
        lv_obj_add_style(obj, &styles->bg_color_primary, LV_PART_ITEMS);
        lv_obj_add_style(obj, &styles->pressed, LV_PART_ITEMS | LV_STATE_PRESSED);
